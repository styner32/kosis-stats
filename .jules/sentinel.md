...
## 2025-02-28 - [High] Prevent DoS by Replacing http.Get with Custom Client with Timeout
**Vulnerability:** The application used the default `http.Get` in `internal/pkg/fred/api.go` for making external requests. The default HTTP client in Go has no timeout, meaning if the external FRED API hangs or becomes unresponsive, the goroutine making the request will block indefinitely. This can lead to connection exhaustion and Denial of Service (DoS).
**Learning:** Even internal API wrapper packages need to enforce timeouts to prevent cascading failures if third-party APIs become unresponsive. Relying on default package-level functions like `http.Get` is a common but dangerous security flaw in Go.
**Prevention:** Always use custom `http.Client` instances with an explicitly configured `Timeout` field (e.g., `&http.Client{Timeout: 20 * time.Second}`). Never use `http.Get`, `http.Post`, etc., directly.
