...
## 2026-03-24 - [Missing Timeout on HTTP Client (http.Get)]
**Vulnerability:** The application used the default `http.Get` in `internal/pkg/binance/api.go` and `internal/pkg/fred/api.go` to make external API calls. The default HTTP client in Go does not have a timeout, meaning it will wait indefinitely for a response if the server hangs or the connection is dropped without a FIN packet, leading to resource exhaustion (goroutine leaks and socket depletion).
**Learning:** Default Go HTTP clients and `http.Get` are unsafe for production use because they lack timeouts. This vulnerability is particularly dangerous when dealing with external APIs that may be slow or unresponsive.
**Prevention:** Always instantiate a custom `http.Client` with an explicit `Timeout` (e.g., `Timeout: 20 * time.Second`) for all outgoing HTTP requests, and prefer passing a context (`http.NewRequestWithContext`) for finer control over cancellation.
