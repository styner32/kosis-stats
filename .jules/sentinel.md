...
## 2026-05-24 - Missing HTTP timeouts causing DoS risk
**Vulnerability:** Usage of default `http.Get` which lacks a timeout.
**Learning:** Default HTTP clients can hang indefinitely if the external server is unresponsive, leading to resource exhaustion (DoS).
**Prevention:** Always use custom `http.Client` with explicit `Timeout` configuration for external API calls.
