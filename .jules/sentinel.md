...
## 2026-03-24 - [Missing Timeout on External API Call]
**Vulnerability:** The application used the default `http.Get` function to make an external API call to FRED. The default HTTP client in Go does not have a timeout configured.
**Learning:** If the external API hangs or responds very slowly, the application will block indefinitely waiting for a response, leading to a build up of goroutines, connection exhaustion, and ultimately a Denial of Service (DoS) for the application. Even if a custom `http.Client` is instantiated with a timeout (as it was in this case on the struct), it must be explicitly used (e.g., `c.client.Get(url)`) instead of the package-level `http.Get`.
**Prevention:** Always use a custom `http.Client` with an explicit `Timeout` configured. Furthermore, ensure the codebase enforces the use of this custom client instead of inadvertently falling back to the default `http.Get` or `http.Post`. Consider adding a linter like `nohttpget` or `gosec` to detect and block usage of default HTTP package functions.
